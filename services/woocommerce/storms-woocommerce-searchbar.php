	...
	class Storms_WC_SearchBar extends WC_Widget
	{
		...
		public function widget($args, $instance)
		{
			...
			<div class="storms-wc-searchbar-container">
				<form method="get" id="storms-wc-searchbar-form" class="<?php esc_attr_e(implode(' ', $class)); ?>"
					...
					<div class="input-group">
						...
						<input class="form-control search-input" type="text"
							   aria-label="<?php _e('Search our products', 'storms'); ?>"
							   value="<?php echo get_search_query(); ?>" name="s" id="s"
							   placeholder="<?php echo esc_attr($label_input); ?>" autocomplete="off"/>

						<?php $loader_image = \StormsFramework\Helper::get_asset_url('/img/spinner.gif'); ?>
						<span class="storms-wc-searchbar-loader-image hidden"
							  style="background-image:url('<?php echo esc_url($loader_image) ?>');"></span>
		...
		private function get_categories_dropdown($label_dropdown, $type, $hide_empty)
		{
			...
			?>
			<button id="storms-wc-searchbar-dropdown"
					class="btn btn-outline-secondary dropdown-toggle categories-dropdown-btn" type="button"
				...
				if ($count > 0) { ?>
					<ul class="parent-list">
						...
						foreach ($categories as $category) { ?>
							...
							<li class="dropdown-item parent-cat"><a
									href="<?php echo esc_attr(get_term_link($category)); ?>">
								...
								if (!empty($category->children)) { ?>
									<ul class="child-list">
										...
										foreach ($category->children as $child) { ?>
											<li class="dropdown-item child-cat"><a
													href="<?php echo esc_attr(get_term_link($child)); ?>">
