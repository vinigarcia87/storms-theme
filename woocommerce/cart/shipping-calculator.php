...
<form class="woocommerce-shipping-calculator <?php echo is_product() ? 'woocommerce-product-shipping-calculator' : ''; ?>" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
	...
	<section class="shipping-calculator-form" <?php echo ! is_product() ? 'style="display:none;"' : ''; ?> >
		...
		<p class="shipping-calculator-action">
			<button type="submit" name="calc_shipping" value="1" class="button<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>">
				...
				<?php else: ?>
					<?php esc_html_e( 'Calcular frete', 'storms' ); ?>
				<?php endif; ?>
			</button>
			<?php if( is_product() ): ?>
		...
		<?php else: ?>
			<?php wp_nonce_field( 'storms-wc-scip-nonce', 'storms-wc-scip-nonce' ); ?>
		<?php endif; ?>
	</section>
</form>
